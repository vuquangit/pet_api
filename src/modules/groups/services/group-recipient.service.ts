...
export class GroupRecipientService implements IGroupRecipientService {
  ...
  async removeGroupRecipient(params: RemoveGroupRecipientParams) {
    ...
    if (!group) throw new GroupNotFoundException();
    // Not group owner
    if (group.owner._id.toString() !== issuerId)
      throw new NotGroupOwnerException();
    // Temporary
    if (group.owner._id.toString() === removeUserId)
      throw new HttpException(
        'Cannot remove yourself as owner',
        HttpStatus.BAD_REQUEST,
      );
    group.users = group.users.filter((u) => u._id.toString() !== removeUserId);
    const savedGroup = await this.groupService.saveGroup(group);
  ...
  async isUserInGroup({ id, userId }: CheckUserGroupParams) {
    const group = await this.groupService.findGroupById(id);
    if (!group) throw new GroupNotFoundException();
    const user = group.users.find((user) => user._id.toString() === userId);
  ...
  async leaveGroup({ id, userId }: LeaveGroupParams) {
    ...
    console.log(group.users.filter((user) => user._id.toString() !== userId));
